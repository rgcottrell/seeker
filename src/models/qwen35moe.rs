...
impl Qwen35MoeModel {
    pub fn new(
        ...
        tracing::info!(
            ...
            attention_layers = (0..params.n_main)
                .filter(|&i| params.is_attention_layer(i))
                .count(),
            ssm_layers = (0..params.n_main)
                .filter(|&i| !params.is_attention_layer(i))
                .count(),
...
impl Model for Qwen35MoeModel {
    ...
    fn scratch_bytes_estimate(
        ...
        // Flash-attn prefill split-K partials on the attention layers: a chunk
        // attending to a long KV prefix splits the KV across `k_num` workgroups,
        // each writing a (head_dim_v+2)·l·n_head partial slice (alloc'd from
        // scratch per FA call, reclaimed per layer). Size for the deepest split;
        // keep `fa_walk` in sync with `flash_attn::prefill_fa_kv_walk()`.
        let fa_walk = 8192u64;
        let fa_partials = if max_seq_len as u64 > fa_walk {
            let fa_k_num = (max_seq_len as u64).div_ceil(fa_walk);
            (p.head_dim_v as u64 + 2) * l * p.n_head as u64 * fa_k_num * 4
        } else {
            0
        };
...
impl Qwen35MoeModel {
    ...
    fn forward_impl(
        ...
        let mut rope_params = rope_multi::RopeMultiParams::qwen_default(
            p.rope_dim,
            p.rope_freq_base,
            p.rope_sections,
        );
    ...
    fn mtp_draft_impl(
        ...
        if h_last.len() as u64 != hidden {
            return Err(
                format!("mtp_draft: h_last len {} != n_embd {hidden}", h_last.len()).into(),
            );
        }
    ...
    fn forward_unified_impl(
        ...
        if positions.len() != tokens.len() || slots.len() != b {
            return Err(
                "forward_unified_impl: tokens/positions/seq_lens/slots length mismatch".into(),
            );
        }
...
fn attention_block(
    ...
    if v_cache_layer.dtype == GgmlType::F16 {
        ...
        crate::inference::command::record_compute_barrier(ctx.device, ctx.cmd, q_roped.range());
    } else {
...
fn ssm_block(
    ...
    let slice_qkv = |chan_offset: u64, chan_count: u64| -> TensorView {
        TensorView {
            ...
            element_stride: [1, conv_channels, conv_channels * l_u, conv_channels * l_u],
    ...
    let gdn_state_in = if let Some(persistent) = gdn_state_persistent {
        ...
        let host_ptr = ctx
            ...
            .ok_or("scratch region not host-visible")?;
        unsafe {
            std::ptr::write_bytes(host_ptr.add(r.offset as usize), 0, r.size as usize);
        }
        r
    };
...
fn attention_block_unified(
    ...
    let q_attn = TensorView {
        ...
        byte_stride: [
            elem,
            elem * head_dim_k * n_head,
            elem * head_dim_k,
            elem * head_dim_k * n_head * n_total,
        ],
        element_stride: [
            1,
            head_dim_k * n_head,
...
fn ssm_block_batch(
    ...
    let conv_input = ctx.alloc_tensor([n_padded, conv_channels, b, 1], GgmlType::F32)?;
    {
        ...
        unsafe {
            std::ptr::write_bytes(
                host.add(conv_input.byte_offset as usize),
                0,
                conv_input.byte_size as usize,
            );
        }
...
fn parse_params(
    gguf: &GgufFile,
    handle: &WeightsHandle,
) -> Result<Qwen35MoeParams, Box<dyn Error>> {
...
mod vision_pos_tests {
    use super::{ImageSpan, build_decoder_mrope_positions, build_decoder_mrope_positions_window};
    ...
    fn text_only_is_sequential_on_all_axes() {
        ...
        for tok in 0..l {
            assert_eq!(
                at(&p, l, tok),
                (tok as u32, tok as u32, tok as u32, tok as u32)
            );
        }
