...
def _build_design(
    ...
    *,
    ...
    When ``fused_silu`` is set, each compute core applies SiLU to its (m, n) output
    tile in-core after the K-loop — a fused GEMM+SiLU epilogue (the FFN gate
    projection). The SiLU microkernel (``silu.cc``) is bf16 and fixed at a
    1024-element tile, so the design requires ``m * n == 1024``.
    """
    ...
    if fused_silu:
        ...
        silu_kernel = kernels.silu(tile_size=m * n)
    ...
    def core_fn(in_a, in_b, out_c, zero, matmul, silu=None):
        ...
        for _ in loop:
            ...
            for _ in range_(K // k):
                ...
                in_b.release(1)
            if silu is not None:
                # Fused epilogue: SiLU the (m, n) output tile in-place on-chip.
                silu(elem_out, elem_out)
            out_c.release(1)

    def _worker_args(row, col):
        args = [
        ...
        if silu_kernel is not None:
            args.append(silu_kernel)
        return args

    workers = Worker.grid(
        n_aie_rows,
        n_aie_cols,
        lambda row, col: Worker(core_fn, _worker_args(row, col), stack_size=0xD00),
    )
...
def _make_argparser():
    ...
    p.add_argument(
        ...
        choices=[0, 1],
        default=0,
        help="fuse a SiLU epilogue (the FFN gate projection); needs m*n == 1024",
    )
...
def _numpy_reference(A_np, B_np, b_col_maj, dtype_out, fused_silu=0):
    ...
    out = (A_np.astype(np.float32) @ B_logical.astype(np.float32)).astype(np.float32)
    if fused_silu:
...
def _run_and_verify(opts):
    ...
    expected_logical = _numpy_reference(A_np, B_np, opts.b_col_maj, dtype_out, opts.silu)
    if opts.c_col_maj:
    ...
    # SiLU is a bf16 LUT (~few % error); loosen the float tolerance when fused.
    extra = dict(float_rtol=0.06, float_atol=0.5) if opts.silu else {}
    assert_close_with_benchmark(
        actual,
        expected,
        bench=bench,
        ops=2.0 * opts.M * opts.K * opts.N,
        fail_msg="output does not match SiLU(A @ B)" if opts.silu else "output does not match A @ B",
