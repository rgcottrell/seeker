//! Shared, backend-free embedding post-processing + the backend-agnostic
//! [`TextEmbedder`] trait.
//!
//! A backend (Vulkan today, NPU next) produces the per-position pre-`output_norm`
//! residual `[n_embd, L]` (position-major); the host-side helpers here turn that
//! into the final embedding(s) — apply the model's `output_norm`, pool over
//! positions, and normalize — matching llama.cpp's *norm-all-then-pool* order.
//! Living in `seeker-core` means every backend and `seeker serve` share one
...
/// A backend that turns token ids into per-position hidden states for embedding.
///
/// The trait is deliberately **high-level / whole-forward**: a single call runs
/// the entire model and returns the pre-`output_norm` residual, leaving the
/// (cheap, backend-agnostic) `output_norm` + pool + normalize to
/// [`pool_and_normalize`] on the host. This is the only granularity that suits
/// the NPU, whose ~100 µs per-dispatch latency makes an op-level trait untenable;
/// it also lets the Vulkan implementation be a thin, byte-identical wrapper over
/// the existing engine path.
pub trait TextEmbedder: Send {
    /// Encode text into token ids using the model's own tokenizer (with special
    /// tokens), so both backends tokenize identically.
    fn tokenize(&self, text: &str) -> Result<Vec<u32>, Box<dyn Error>>;

    /// Optional hint: pre-size internal scratch for sequences up to `max_seq_len`
    /// so a batch of inputs doesn't repeatedly grow buffers. Default: no-op.
    /// [`embed_residual`](TextEmbedder::embed_residual) must still work without it.
    fn reserve(&mut self, _max_seq_len: u32) -> Result<(), Box<dyn Error>> {
        Ok(())
    }

    /// Run the full forward for one text and return the pre-`output_norm`
    /// residual as `[n_embd * L]`, position-major (token `t` at
    /// `residual[t*n_embd .. (t+1)*n_embd]`) — the same layout the host
    /// [`pool_and_normalize`] consumes.
